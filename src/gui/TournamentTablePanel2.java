 ...
 *
 * some layout modifications (c) Ryan Lagerquist
 */
...
public class TournamentTablePanel2 extends TablePanelBase
  implements EventHandler,WindowListener,ComponentListener
{
  ...
  CardImages cardImagesTrickSmall, cardImagesLastTrick, cardImagesTrickBig;
  ...
  public TournamentTablePanel2(ClientWindow clientWindow_,
                               JFrame tableFrame_,
                               String clientName_,
                               Table table_,
                               ...
                               String invites)
  {
    ...
    tournamentResultTable = new TournamentResultTable2(table);
    ...
    switchTButton.setEnabled(true);
    switchGButton.setEnabled(true);
    ...
    switchTButton.setMargin(new java.awt.Insets(2, 5, 2, 5));
    switchGButton.setMargin(new java.awt.Insets(2, 5, 2, 5));        
    ...
    switchTButton.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          Misc.msg("switchT clicked");
          removeAll();
          setLayout(new MigLayout("ins 0, fill", "[grow][fill]", "[grow][fill]"));
          add(gameTab, "growx, growy");
          revalidate();
          validate();
	}
      });
    switchGButton.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          Misc.msg("switchG clicked");
          removeAll();
          setLayout(new MigLayout("ins 0, fill", "[grow][fill]", "[grow][fill]"));          
          add(talkTab, "growx, growy");
          revalidate();
	}
      });
  ...
  void setCardImages()
  {
    ...
    cardImagesLastTrick  = getCardImages(trickCardPanel.getUsableWidth() / 12); // a bit bigger
  ...
  private void setHandTitles(SimpleState st, int myIndex)
  {
    ...
    for (int p=0; p < 3; p++) { // player index in game
	  ...
	  if (ste.getDeclarer() == p) {
		  ...
		  titleB += getModifiers(ste.getGameDeclaration()); // solves i18n issue in modifiersToVerboseString()
	      }
	  } else {
	      titleB += rbs("Def.");
	  }
  ...
  public void game2Window(ClientWindow.TablePanelUpdateAction action, String[] params)
  {
	...
	tournamentResultTable.resize(scale);
      ...
      switchTButton.setText(rbs("See_Game") + " " + table.getGameNum());
      // switchTButton.setText(rbs0("See Game") + " " + table.getGameNum());
      ...
      if (action == ClientWindow.TablePanelUpdateAction.MOVE) {
        ...
        // switch to game view when move arrives
        ...
        if (!noob) {
          ...
          if (imp.size() > 0) {
            ...
            if (i >= 0) {
              ...
              // !!! was cardImagesTrickSmall
              ArrayList<ImagePos> imp2 = Utils.cardsToImages(prevTrickCards, cardImagesLastTrick);
              cw2 = cardImagesLastTrick.getWidth();  // card width
              ch2 = cardImagesLastTrick.getHeight(); // card height            
  ...
  private JPanel getGameTab() {
    ...
    topRightPanel.add(gameNumberLabel,  "sgx 1, wrap 5");
    // topRightPanel.add(switchGButton,    "sgx 1, wrap");
  ...
  private JPanel getTalkTab() {
    ...
    //JPanel talkPanel = new JPanel(new MigLayout("ins 0, fill", "[grow]", "[grow][grow]"));

    JPanel talkPanel = new JPanel(new MigLayout("ins 0, flowy, fill", "[grow]", "[grow][grow]"));
    JPanel bottomRightPanel = new JPanel(new MigLayout("ins 0, fill", "[shrink]", "[shrink][shrink][shrink]"));
    resultScrollPane.setViewportView(resultTable);
    bottomRightPanel.add(resultScrollPane, "span 5, center, growy, wrap");

    JPanel readyInvitePanel = new JPanel(new MigLayout("ins 0", "[shrink]", "[shrink][shrink][shrink]"));
    // may need to "fill" here

    bottomRightPanel.add(helpButton, "span 5, split 5, center");
    bottomRightPanel.add(leaveButton);
    bottomRightPanel.add(resizeLabel);
    bottomRightPanel.add(plusButton);
    bottomRightPanel.add(minusButton, "wrap");
    // bottomRightPanel.add(switchTButton, "span 5, split 2, center");
    bottomRightPanel.add(readyButton, "center, gap 20");
    ...
    tournamentResultPanel.add(legend,                      "center, w 100%, span");
  ...
  private void initComponents()
  {
    ...
    tableTabbedPane = new JTabbedPane(JTabbedPane.RIGHT);
    ...
    switchTButton = new JButton();
    switchGButton = new JButton();    
    ...
    // resizeLabel = new JLabel(rbs0("Resize"));
    resizeLabel = new JLabel(rbs("Resize"));
    ...
    switchTButton.setText(rbs("See_Game"));
    // switchTButton.setText(rbs0("See Game"));
    switchGButton.setText(rbs("See_Results"));
    // switchGButton.setText(rbs0("See Results"));
    ...
    */
    ...
    tableTabbedPane.setLayout(new MigLayout("ins 0, flowy, fill", "[grow]", "[grow]"));

    // vertical labels hack
    // tableTabbedPane.addTab(rbs0("<html><center>R<br>e<br>s<br>u<br>l<br>t<br>s</center></html>"), talkTab);
    // tableTabbedPane.addTab(rbs0("<html><center>G<br>a<br>m<br>e</center></html>"), gameTab);
    tableTabbedPane.addTab(rbs("<html><center>R<br>e<br>s<br>u<br>l<br>t<br>s</center></html>"), talkTab);
    tableTabbedPane.addTab(rbs("<html><center>G<br>a<br>m<br>e</center></html>"), gameTab);
  ...
  JButton leaveButton;
  JButton switchTButton, switchGButton;
  JButton readyButton;
  ...
  JLabel legend; // legend for tournament-style result table
  ...
  TournamentResultTable2 tournamentResultTable;

  TournamentTablePanel2 getThis() { return this; }
  ...
  public void resize(float f)
  {
    ...
    tournamentResultTable.resize(f);
