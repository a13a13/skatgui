 ...
 *
 * Layout of bidding panel, with increment and decrement buttons, modified.
 * - Ryan Lagerquist
 */
...
public class TablePanel extends TablePanelBase
  implements EventHandler,WindowListener,ComponentListener
{
  ClientWindow clientWindow;
  JFrame tableFrame; // for dialogs
  String clientName;
  String playerName;
  String cardDeckPath;
  String cardDeck;
  ...
  int numIncrements = 0; // to allow bidder to increment/decrement next bid - Ryan
  ...
  /* An HTML result table, including the details of all games played in the series,
     will be saved to the directory in which the user has installed skatgui when he
     leaves the table.  This needs to be made more robust so that if the user leaves
     the table in an unexpected way -- e.g., shuts down the program from the Task
     Manager or becomes disconnected -- the result table will still be printed and
     saved. - Ryan */
  HTMLResultTable htmlResults;
  
  /** Creates new form TablePanel */
  public TablePanel(ClientWindow clientWindow_,
                    JFrame tableFrame_,
                    String clientName_,
                    Table table_,
                    ...
                    String invites)
  {
    ...
    //    setTitle(clientName + " " + (isPlayer ? rbs("Player") + " " + playerName : rbs("Observer")) + " @ "+
    //         rbs("Table") + " " + table.getId() + " " +
    //         rbs("Type") + " " + table.getType());
      
    // hook up components

    noobCheckBox.setSelected(false);
    ...
    leaveButton.setMargin(new java.awt.Insets(2, 5, 2, 5));
    listButton.setMargin(new java.awt.Insets(2, 5, 2, 5));        
    ...
    resultTable.getColumnModel().getColumn(3).setCellRenderer(center);
    ...
    resultTable.getColumnModel().getColumn(7).setCellRenderer(right);
    resultTable.getColumnModel().getColumn(8).setCellRenderer(center);
    resultTable.getColumnModel().getColumn(9).setCellRenderer(center);
    resultTable.getColumnModel().getColumn(10).setCellRenderer(center);
        ...
        public void actionPerformed(ActionEvent e) {
          ...
          synchronized (clientWindow.sc) {
            ...
            if (sg != null) {
              ...
              int myIndex = sg.getPlayerIndex(playerName);
              if (myIndex < 0)
                panelIndex = -1;
              else
                panelIndex = 0;
        ...
        @Override
          ...
          synchronized (clientWindow.sc) {          
            ...
            if (st.getPhase() == SimpleState.BID) {
              ...
              if (st.nextBidInc(numIncrements) >= 0) {

                send("play " + st.nextBidInc(numIncrements)); // send bid
                lastBeep = 0; // action stops beep timer
              }
    ...
    bidIncrementButton.addMouseListener(new MouseAdapter() {
        @Override
          ...
          synchronized(clientWindow.sc) {
            ...
            if (st.nextBidInc(++numIncrements) < 0) --numIncrements;
            if (st.nextBidInc(++numIncrements) < 0) --numIncrements;
            if (st.nextBidInc(++numIncrements) < 0) --numIncrements;
            if (st.nextBidInc(++numIncrements) < 0) --numIncrements;            
    ...
    leaveButton.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          htmlResults = new HTMLResultTable(table); // need to add condition for this later
          // if (!table.getTourny() || table.isFinished()) // cannot save table archive file for non-tourney table
    ...
    seatsButton.addMouseListener(new MouseAdapter() {
        @Override
          public void mouseClicked(MouseEvent evt) {
          Misc.msg("34 clicked");
          send("34");
    ...
    inviteButton.addMouseListener(new MouseAdapter() {
        @Override        
          ...
          inviteWindow.setLocationRelativeTo(null);
          inviteWindow.setLocation(getPanel().getLocationOnScreen().x +
                                   clientWindow.getOrigWidth()/4,
                                   getPanel().getLocationOnScreen().y +
                                   clientWindow.getOrigHeight()/4);

          inviteWindow.update(clientWindow.getUserData());
    ...
    showMainButton.addMouseListener(new MouseAdapter() {
        @Override        
          public void mouseClicked(MouseEvent evt) {
          Misc.msg("show main clicked");
          
          clientWindow.mainTabbedPane.setSelectedIndex(2); // main tab
  ...
  String getModifiers(GameDeclaration decl) {
    // internationalizes modifiers returned by modifiersToVerboseString() in GameDeclaration
	
    if (decl.modifiersToVerboseString().equals(" Ouvert"))
      return " " + rbs("Ouvert");
    if (decl.modifiersToVerboseString().equals(" Hand"))
      return " " + rbs("Hand");
    if (decl.modifiersToVerboseString().equals(" Ouvert Hand"))
      return " " + rbs("Ouvert") + " " + rbs("Hand");
    if (decl.modifiersToVerboseString().equals(" Schneider"))
      return " " + rbs("Schneider");
    if (decl.modifiersToVerboseString().equals(" Schwarz"))
      return " " + rbs("Schwarz");
    if (decl.modifiersToVerboseString().equals(" Hand Schneider"))
      return " " + rbs("Hand") + " " + rbs("Schneider");
    if (decl.modifiersToVerboseString().equals(" Hand Schwarz"))
      return " " + rbs("Hand") + " " + rbs("Schwarz");
    else
      return "";
  }
      ...
      if (ste.getDeclarer() >= 0) {
        ...
        if (ste.getDeclarer() == p) {
          ...
          if (ste.getGameDeclaration().type != GameDeclaration.NO_GAME) {
            ...
            titleB += getModifiers(ste.getGameDeclaration()); // solves i18n issue with modifiersToVerboseString()
	  }
        } else {
          titleB += rbs("Def.");
        }
  ...
  void setDeclarationButtons()
  {
    ...
    if (!sg.isFinished()) {
      ...
      case SimpleState.ANSWER:
        ...
        bidDecrementButton.setEnabled(false); // increment and decrement buttons are disabled
        bidIncrementButton.setEnabled(false); // when the user is merely answering a bid - Ryan
        break;
  ...
  public void game2Window(ClientWindow.TablePanelUpdateAction action, String[] params)
  {
    synchronized (clientWindow.sc) {
      ...
      if (action == ClientWindow.TablePanelUpdateAction.TIMER) {
        ...
        for (;;) { // for break
          ...
          if ((lastBeep % 25) == 1) { // beep after x seconds
            if (clientWindow.getSettings().beep != 0)
        ...
        JOptionPane.showMessageDialog(tableFrame,
                                      "<html><b>"+clientWindow.translate(params[0])+"</html>",
                                      rbs("Table_Error_Message"),
                                      JOptionPane.ERROR_MESSAGE);
        ...
        clubsButton.doClick();
        gameType = GameDeclaration.CLUBS_GAME;
        listWindow.update();  // update names
      ...
      gameNumberLabel.setText(rbs("Game") + " " + table.getGameNum() + (table.getMaxGameNum() > 0 ?
                                                                        "/" + table.getMaxGameNum() : ""));
      ...
      for (int i=0; i < Table.getMaxPlayerNum(); i++) {
        ...
        if (table.getPlayerAtTable(i) == null) {
          ...
          resultTable.getModel().setValueAt("", i, 3);
        } else {
          resultTable.getModel().setValueAt(p, i, 0);
          ...
          resultTable.getModel().setValueAt(p, i, 3);
        }

        // IP set ids
        resultTable.getModel().setValueAt(table.getIPsetIdInResultTable(i)+"", i, 2);
        
        // wins
        resultTable.getModel().setValueAt(table.getWins(i)+"", i, 4);
      
        // losses
        resultTable.getModel().setValueAt((table.getPlayed(i)-table.getWins(i))+"", i, 5);
      
        // last points
        resultTable.getModel().setValueAt(table.getLastPts(i)+"", i, 6);
      
        // total points
        resultTable.getModel().setValueAt(table.getTotalPts(i)+"", i, 7);
      
        // 34
        resultTable.getModel().setValueAt(table.get34(i) ? "X" : "", i, 8);

        // gametalk
        resultTable.getModel().setValueAt(table.getGameTalk(i) ? "X" : "", i, 9);

        // ready
        resultTable.getModel().setValueAt(table.getReady(i) ? "X" : "", i, 10);
      }
      ...
      if (action == ClientWindow.TablePanelUpdateAction.MOVE) {
        ...
        // added to distinguish MOVES that triggered update (RE in particular)
        // important because window update is asynchronous, and so the table may have been changed
        // twice before the first update is executed
        ...
        //Misc.msg("ACTION MOVE: " + params[0] + " " + params[1] + " " + sg.toSgf(false, sg.getOwner()));
	...
	/* (for Ryan's testing purposes -- please leave in)
	// saving game in .sgf format
	for (int index = 0; index < Integer.MAX_VALUE; index++) {
        outputFileName.delete(0, outputFileName.length());
        outputFileName.append("sgf_files/game");
        outputFileName.append(index);
        outputFileName.append(".sgf");
	...
	try {
        ...
        htmlWriter.print(g.toSgf(false, SimpleState.WORLD_VIEW));
        htmlWriter.close();
	}
	catch (IOException e) {
	...
	*/
      ...
      boolean noob = noobCheckBox.isSelected() || sg.isFinished() ||
        sg.getOwner() == SimpleState.WORLD_VIEW;
      ...
      seatsButton.setEnabled(mode == DisplayMode.HISTORY);
      talkButton.setEnabled(sg.getPlayerIndex(playerName) >= 0 || mode == DisplayMode.HISTORY);
        ...
        if (mode != DisplayMode.HISTORY) {
          ...
          if (meToMove) {
            ...
            if (phase == SimpleState.BID) {
              ...
              bidButton1.setText(""+s.nextBidInc(numIncrements));

              Misc.msg("BID TEXT: " + s.nextBidInc(numIncrements) + " " + numIncrements + " " +
                       s.nextBid());
  ...
  {
    if (color == null) 
      showMainButton.setBackground(Color.GREEN);
    else
      showMainButton.setBackground(color);      
  }


  private JPanel getTopPanel()
  {
    // suit panels within card panels

    trickCardPanel.setLayout(new org.netbeans.lib.awtextra.AbsoluteLayout());

    declarationPanel.setLayout(new MigLayout("nogrid,center"));
    ...
    biddingPanel.setLayout(new MigLayout("nogrid, center"));
    ...
    // biddingPanel.setLayout(new MigLayout("nogrid,center"));
    // biddingPanel.add(biddingLabel, "wrap");
    // biddingPanel.add(bidButton1, "center, split");
    // biddingPanel.add(bidButton2, "wrap");
    // biddingPanel.add(bidIncrementButton, "center, split");
    // biddingPanel.add(bidDecrementButton);    
    
    ////////////////

    JPanel topPanel = new JPanel(new MigLayout("ins 0, fill", "[83%][17%]", "[grow]"));

    JPanel topLeftPanel = new JPanel(new MigLayout("ins 0, fill", "[grow]", "[grow][grow][grow]"));

    /*
      topLeftPanel.add(new JPanel(), "growx, growy");
      topLeftPanel.add(new JPanel(), "growx, growy, wrap");
      topLeftPanel.add(trickCardPanel, "growx, growy, span 2, wrap");
      topLeftPanel.add(new JPanel(), "growx, growy, span 2");
    */
    
    topLeftPanel.add(cardPanel1,     "w 100%, h 26%");
    topLeftPanel.add(cardPanel2,     "w 100%, h 26%, wrap");
    topLeftPanel.add(trickCardPanel, "w 100%, h 39%, span 2, wrap");
    topLeftPanel.add(cardPanel0,     "w 100%, h 35%, span 2");

    JPanel topRightPanel = new JPanel(new MigLayout("nogrid, ins 0, fillx", "[center]"));

    topRightPanel.add(showButton,       "sgx 1, w 100%, wrap");
    topRightPanel.add(resignButton,     "sgx 1, wrap 8");
    topRightPanel.add(declarationPanel, "sgx 1, wrap 8");
    topRightPanel.add(biddingPanel,     "sgx 1, wrap 8");
    topRightPanel.add(playingLabel,     "sgx 1, wrap 3");
    topRightPanel.add(gameNumberLabel,  "sgx 1, wrap");

    topPanel.add(topLeftPanel,  "growx, growy");
    topPanel.add(topRightPanel, "w 100%, growy");
  ...
  {
    JPanel bottomPanel = new JPanel(new MigLayout("nogrid, ins 0, fill", "[grow][center,grow]"));

    JPanel bottomRightPanel = new JPanel(new MigLayout("nogrid, ins 0, fill", "[center]"));

    //JLimitedTextArea textArea = new JLimitedTextArea();
    //JHistTextField inputLine = new JHistTextField();
    ...
    bottomRightPanel.add(resultScrollPane, "wrap");
    bottomRightPanel.add(noobCheckBox);
    bottomRightPanel.add(inviteButton);
    bottomRightPanel.add(seatsButton);
    bottomRightPanel.add(talkButton);
    bottomRightPanel.add(readyButton, "wrap");
    bottomRightPanel.add(listButton);    
    bottomRightPanel.add(leaveButton);
    bottomRightPanel.add(plusButton);
    bottomRightPanel.add(minusButton);             
    bottomRightPanel.add(helpButton);
    bottomRightPanel.add(showMainButton);

    JPanel bottomLeftPanel = new JPanel(new MigLayout("ins 0, flowy, fill", "[grow][]"));
    
    bottomLeftPanel.add(textScrollPane, "growx, growy");
    bottomLeftPanel.add(inputLine, "growx");
    
    bottomPanel.add(bottomLeftPanel, "growx, growy");
    bottomPanel.add(bottomRightPanel, "growy");
    ...
    /*
      ...
      JPanel bottomRightPanel = new JPanel(new MigLayout("nogrid,fillx,ins 0", "[center]"));
      ...
      JPanel bottomPanel = new JPanel(new MigLayout("nogrid,fillx, ins 0", "[center]")); //"filly,nogrid", "[fill]" for resizable cont.

      JPanel panel = new JPanel(new MigLayout("nogrid, ins 0, fill"));

      panel.add(textScrollPane, "growx,growy,wrap");
      panel.add(inputLine, "growx");
    
      bottomPanel.add(panel, "growx,growy");
  ...
  private void initComponents()
  {
    ...
    noobCheckBox = new JCheckBox();
    inviteButton = new JButton();
    plusButton = new JButton();
    minusButton = new JButton();    
    seatsButton = new JButton();
    ...
    declarationPanel.setBorder(BorderFactory.createTitledBorder(rbs("Contract")));

    gameTypeButtonGroup.add(nullButton);
    nullButton.setText(rbs("Null"));
    ...
    handCheckBox.setText(rbs("Hand"));
    schneiderCheckBox.setText("Schneider");
    schwarzCheckBox.setText("Schwarz");

    leaveButton.setText(rbs("Leave"));
    listButton.setText(rbs("List"));    
    helpButton.setText(rbs("Help"));

    noobCheckBox.setText(rbs("I'm_a_Noob")); // NOI18N
    noobCheckBox.setToolTipText(rbs("Show_complete_game_history")); // NOI18N
    noobCheckBox.setBorder(BorderFactory.createEmptyBorder(0, 0, 0, 0));
    noobCheckBox.setMargin(new java.awt.Insets(0, 0, 0, 0));

    showMainButton.setText(rbs("Button-Show-Main"));
    showMainButton.setToolTipText(rbs("Tooltip-Show-Main"));
    showMainButton.setMargin(new java.awt.Insets(2, 5, 2, 5));
    ...
    inviteButton.setMargin(new java.awt.Insets(2, 5, 2, 5));

    seatsButton.setText("34");
    seatsButton.setToolTipText(rbs("Tooltip-34")); // NOI18N
    seatsButton.setMargin(new java.awt.Insets(2, 5, 2, 5));

    talkButton.setText(rbs("Button-GameTalk")); // NOI18N
    talkButton.setToolTipText(rbs("Tooltip-GameTalk")); // NOI18N
    talkButton.setMargin(new java.awt.Insets(2, 5, 2, 5));

    readyButton.setText(rbs("Button-Ready")); // NOI18N
    readyButton.setToolTipText(rbs("Tooltip-Ready")); // NOI18N
    readyButton.setMargin(new java.awt.Insets(2, 5, 2, 5));
    ...
    listWindow = new ListWindow(tableFrame, table,
                                clientWindow.getOrigWidth(),
                                clientWindow.getOrigHeight());

    inviteWindow = new InviteWindow(tableFrame, this, table,
                                    clientWindow.getOrigWidth()/2,
                                    clientWindow.getOrigHeight()/2);
    ...
    /*
      setLayout(new MigLayout("debug, ins 0, flowy, fill"));
    
      add(getTopPanel(), "h 75%");
      add(getBottomPanel(), "h 25%, growx");
    */
  ...
  JButton seatsButton, showButton, talkButton;
  ...
  JLabel diamondsLabel, diamondsLabel0, diamondsLabel1, diamondsLabel2;
  JLabel heartsLabel, heartsLabel0, heartsLabel1, heartsLabel2;
  JLabel spadesLabel, spadesLabel0, spadesLabel1, spadesLabel2;
  JLabel clubsLabel, clubsLabel0, clubsLabel1, clubsLabel2;
  ...
  JScrollPane textScrollPane, resultScrollPane;
  ...
  TablePanel getThis() { return this; }
  ...
  public void resize(float f)
  {
    ...
    //Font f2m = new Font("Lucida Sans Typewriter", 1, Math.round(12*f));    
    ...
    bidIncrementButton.setFont(f1); // f5 too small when window gets smaller
    ...
    if (false) {
      ...
      // measure panel 1/2 and set sizes
      Dimension dim = new Dimension(w/2, 400);
      cardPanel1.setMinimumSize(dim); cardPanel1.setPreferredSize(dim); cardPanel1.setMaximumSize(dim);
      cardPanel1.setSize(dim); cardPanel1.invalidate(); cardPanel1.validate();
      int uh = cardPanel1.getUsableHeight() - yOffsets[1]; // measure offset, and use it maintain ratio
      dim = new Dimension(w/2, 400-uh + Math.round(WIDTH/9.0f*f));
      cardPanel1.setMinimumSize(dim); cardPanel1.setPreferredSize(dim); cardPanel1.setMaximumSize(dim);
      cardPanel1.setSize(dim); cardPanel1.invalidate(); cardPanel1.validate();
      cardPanel2.setMinimumSize(dim); cardPanel2.setPreferredSize(dim); cardPanel2.setMaximumSize(dim);
      cardPanel2.setSize(dim); cardPanel2.invalidate(); cardPanel2.validate();    

      // measure panel 0 and set size    
      dim = new Dimension(w, 400);
      cardPanel0.setMinimumSize(dim); cardPanel0.setPreferredSize(dim); cardPanel0.setMaximumSize(dim);
      cardPanel0.setSize(dim); cardPanel0.invalidate(); cardPanel0.validate();
      uh = cardPanel0.getUsableHeight() - yOffsets[0]; // measure offset, and use it maintain ratio
      dim = new Dimension(w, 400-uh + Math.round(WIDTH/4.8f*f));
      cardPanel0.setMinimumSize(dim); cardPanel0.setPreferredSize(dim); cardPanel0.setMaximumSize(dim);
      cardPanel0.setSize(dim); cardPanel0.invalidate(); cardPanel0.validate();

      // measure trick panel and set size
      dim = new Dimension(w, 400);
      trickCardPanel.setMinimumSize(dim); trickCardPanel.setPreferredSize(dim); trickCardPanel.setMaximumSize(dim);
      trickCardPanel.setSize(dim); trickCardPanel.invalidate(); trickCardPanel.validate();
      uh = trickCardPanel.getUsableHeight(); // measure offset, and use it maintain ratio
      dim = new Dimension(w, 400-uh + Math.round(WIDTH/4.5f*f));
      trickCardPanel.setMinimumSize(dim); trickCardPanel.setPreferredSize(dim); trickCardPanel.setMaximumSize(dim);
      trickCardPanel.setSize(dim);
      trickCardPanel.invalidate(); trickCardPanel.validate();

      trickCardPanel.setMinimumSize(dim); trickCardPanel.setPreferredSize(dim); trickCardPanel.setMaximumSize(dim);
    }
    
    // Misc.msg("TW=" + tw + " CP0 " + dim.width + " " + dim.height + " " + (dim.width / (dim.height+0.0f)));
    // Misc.msg("USE T=" + trickCardPanel.getUsableWidth() + " " + trickCardPanel.getUsableHeight() + " " +
    // trickCardPanel.getUsableWidth() / (trickCardPanel.getUsableHeight()+0.0f));
    ...
    String[] resultCont = new String[] {
      "MMMMxxxx", "EDS", "M", "M", "999", "999", "-999", "9999", "X", "X", "X" };
